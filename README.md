# Appium UI 自动化测试框架（BYR App Demo）

基于 **Appium + Java + TestNG + Allure** 的安卓真机 UI 自动化框架。被测应用是 BYR（北邮人论坛，`com.byr.bbs_flutter`，**Flutter 应用**）。

**核心用例**：点击桌面 BYR 图标 → 进入首页 → 点击「发现」→ 进入发现页 → 从左边缘右滑退出到桌面。
...
│   └── testcase/DemoTest.java  # 用例（流程 + 断言）
└── resources/config.properties # 配置（设备 UDID、坐标、包名等）
```
...
# 跑测试（结果自动采集到 target/allure-results）
mvn test
...
- 依赖：`java-client 8.5.1`、`testng 7.8.0`、`allure-testng 2.24.0`
...
- `BaseTest`：初始化 driver + 通用操作（`tapCoord` / `swipeFromLeftEdge` / `waitAppLaunched` / `saveScreenshot`）
- `TestConfig` + `config.properties`：设备、坐标、包名等配置
- `HomePage`：元素/坐标定位
- `DemoTest`：用例 + 断言
...
跑完看 `target/` 下的截图（`1_home_page.png`、`2_discover_page.png`、`3_back_to_launcher.png`）和 `target/allure-report/index.html` 报告。
